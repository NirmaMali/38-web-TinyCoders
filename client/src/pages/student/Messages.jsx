...
export default function StudentMessages() {
  ...
  useEffect(() => {
    if (activeChat) fetchConversation(activeChat);
  }, [activeChat]);
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      await api.post('/messages/send', { receiverId: activeChat, content: newMsg.trim() });
      setNewMsg('');
      await fetchConversation(activeChat);
      await fetchInbox();
    } catch {}
    setSending(false);
  };
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex h-[calc(100vh-220px)] overflow-hidden">
        ...
        <div className={`w-full sm:w-80 border-r border-gray-100 flex flex-col ${activeChat ? 'hidden sm:flex' : 'flex'}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? <div className="p-4"><CardSkeleton count={3} /></div> : conversations.length === 0 ? (
              <EmptyState title="No messages" description="Start a conversation from the Alumni page." icon={MessageSquare} />
          ...
          {activeChat ? (
            <>
              <div className="p-4 border-b border-gray-100 flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-sm text-gray-900">{partner?.name || 'Loading...'}</p>
                ...
                {messages.map((msg) => (
                  <motion.div key={msg._id} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className={`max-w-[70%] px-4 py-2.5 rounded-2xl text-sm ${msg.senderId._id === user?._id ? 'bg-primary-700 text-white rounded-br-md' : 'bg-gray-100 text-gray-800 rounded-bl-md'}`}>
                      <p>{msg.content}</p>
                      <p className={`text-xs mt-1 ${msg.senderId._id === user?._id ? 'text-primary-200' : 'text-gray-400'}`}>
              ...
              <div className="p-4 border-t border-gray-100 flex gap-2">
                ...
                <button onClick={handleSend} disabled={sending || !newMsg.trim()}
                  className="px-4 py-2.5 bg-primary-700 text-white rounded-xl hover:bg-primary-800 disabled:opacity-50 transition-colors">
            ...
            <div className="flex-1 flex items-center justify-center">
              <EmptyState title="Select a conversation" description="Choose a conversation from the list or start a new one from the Alumni page." icon={MessageSquare} />
            </div>
          )}
        </div>
      </div>
